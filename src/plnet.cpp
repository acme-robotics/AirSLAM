...
bool PLNet::build() {
  if (deserialize_engine()) {
    ...
    // TensorRT 10: binding indices are gone; tensors are addressed by name at
    // infer() time (setInputShape / getHostBuffer), so nothing to cache here.
    if (!context1_) {
  ...
  // TensorRT 10: tensors addressed by name at infer() time; no indices to cache.
  return true;
}
...
bool PLNet::infer(const cv::Mat &image, Eigen::Matrix<float, 259, Eigen::Dynamic> &features, 
  ...
  bool status = context0_->enqueueV3(0);
  if (!status) {
    return false;
  }
...
bool PLNet::process_output(const BufferManager &buffers, Eigen::Matrix<float, 259, Eigen::Dynamic> &features, 
  ...
  bool status = context1_->enqueueV3(0);
  if (!status) {
    return false;
  }
  cudaStreamSynchronize(0);
